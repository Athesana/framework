 ...
 * 	- 인터셉터는 DispatcherServlet 수행 후 컨트롤러에 요청을 넘기기 전에 실행된다. Spring 자원을 이용할 수 있다. (servlet-context.xml에 설정, web에 대한 설정이기 때문)
 * 
 * 인터셉터와 필터를 사용하고,
 * 비즈니스 로직에 대해서 적용해야 할 부분이 있다면, AOP를 사용한다. (Service의 클래스 앞 뒤로 감싸는 역할)
 * 
 */
...
public class LoginCheckInterceptor extends HandlerInterceptorAdapter {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
			throws Exception {
		// 컨트롤러의 처리가 끝나고 화면(View) 처리까지 모두 완료되면 실행되는 메소드
